package org.example.msvcchat.service;

import jakarta.persistence.EntityManager;
import jakarta.persistence.PersistenceContext;
import org.example.msvcchat.client.GroqClient;
import org.example.msvcchat.dto.RespuestaApi;
...
// y que termine en ';'. El DOTALL permite capturar saltos de línea.
    ...
    private String cargarEsquemaSQL(String nombreArchivo) {
        try (InputStream inputStream = new ClassPathResource(nombreArchivo).getInputStream()) {
            ...
            throw new RuntimeException("Error al leer el archivo SQL desde resources: " + e.getMessage(),
                    e);
        }
    }
    /**
...
// ====================================================================
                if (sql.trim().regionMatches(true, 0, "SELECT", 0, 6)) {
                    ...
                    return ResponseEntity.ok(new RespuestaApi<>(true, "Sentencia DML ejecutada con éxito",
                            data));
                }
            } catch (Exception e) {
                log.warn("Error al ejecutar SQL: {}", e.getMessage(), e);
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                        .body(new RespuestaApi<>(false, "Error al ejecutar la sentencia: " +
                                e.getMessage(), null));
            }
        } catch (Exception e) {
